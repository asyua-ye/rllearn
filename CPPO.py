...
class cppo():
    ...
    def compute_loss(self, sample):
        ...
        # action_loss = self.action_loss_weight*action_loss
    ...
    def learn(self, rollout):
        ...
        for e in range(self.ppo_epoch):
            ...
            for sample in data_generator:
                action_loss, value_loss = self.compute_loss(sample)
                ...
                # # 获取所有模型参数
                # parameters = [p for p in self.ACTOR.parameters()]

                # # 检查每个参数是否包含NaN或Inf
                # for param in parameters:
                #     if torch.isnan(param).any() or torch.isinf(param).any():
                #         print(f'NaN or Inf found in {param}')
...
def main():
    ...
    if  not TEST:
        ...
        writer = SummaryWriter(f'{SAVE_PATH_PREFIX}/{datetime.datetime.now().strftime("%Y-%m-%dT%H-%M-%S")}-CPPO-{EPISODES}')
