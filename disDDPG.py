...
class DDPG(object):
    ...
    def learn(self, batch_size):
        ...
        critic_loss,actor_loss=self.compute_loss(sample)
    ...
    def predict(self,observation: Union[np.ndarray, Dict[str, np.ndarray]],state,episode_start,deterministic):
        ...
        if not TEST:
            ...
            action = np.array([self.get_action(observation).view(action_dim).cpu().data.numpy() for _ in range(n_batch)])
    ...
    for i in range(EPISODES):
        ...
        if TEST:
            ...
            while(True):
                ...
                next_state, reward, done, truncated, info = env.step(action.cpu().data.numpy())  # observe next state and reward
                state=next_state
                ep_reward+=reward
                env.render()
                if done:
